(function () {
  ...
  function originStrIsLoopback(originStr) {
    ...
    try {
      return hostIsLoopback(new URL(originStr).hostname);
    } catch (e) {
      return false;
    }
  }
  ...
  function resolveSparkScriptElement() {
    ...
    if (
      cur &&
      cur.src &&
      (cur.src.indexOf('spark.js') !== -1 || cur.src.indexOf('scout.js') !== -1)
    ) {
      return cur;
    }
    ...
    for (j = 0; j < list.length; j++) {
      try {
        if (new URL(list[j].src).origin === pageOrigin) return list[j];
      } catch (e1) {}
    }
    var pageHost = '';
    ...
    if (!hostIsLoopback(pageHost)) {
      for (j = list.length - 1; j >= 0; j--) {
        try {
          if (!hostIsLoopback(new URL(list[j].src).hostname)) return list[j];
        } catch (e3) {}
      }
    }
  ...
  function scriptParams() {
    ...
    var out = {
      api_base: '', title: 'More from Zipline Spark', theme: 'dark',
      drawer_width: '420', scriptEl: script || null,
      adsense_client: '', adsense_slot: '',
      afs_pub_id: '', afs_style_id: '', afs_channel_id: '',
      channel_id: '', pubid: '',
    ...
    if (script && script.src) {
      ...
      if (qStart !== -1) {
        script.src.slice(qStart + 1).split('&').forEach(function (pair) {
          ...
          if (k === 'subid1') out.subid1 = v;
          if (k === 'subid2') out.subid2 = v;
          if (k === 'subid3') out.subid3 = v;
          if (k === 'subid4') out.subid4 = v;
          if (k === 'subid5') out.subid5 = v;
        });
      }
  ...
  function sessionId() {
    try {
      ...
      if (legacy) {
        ...
        try { localStorage.removeItem('zipline_scout_sid'); } catch (e2) {}
        return legacy;
      }
  ...
  function boltSvgMarkup(cls) {
    var classAttr = cls ? ' class="' + escapeHtml(String(cls)) + '"' : '';
    return (
      '<svg' + classAttr + ' viewBox="0 0 483.73 483.73" aria-hidden="true">' +
      '<polygon fill="currentColor" points="119.637,282.441 192.449,282.441 165.869,483.73 364.094,189.622 296.631,189.622 325.678,0"></polygon>' +
      '</svg>'
    );
  }
  ...
  var pageHost = '';
  try { pageHost = window.location.hostname || ''; } catch (ePage) {}
  if (
    fromAttr && originStrIsLoopback(fromAttr) &&
    fromScriptOrigin && !originStrIsLoopback(fromScriptOrigin) &&
    !hostIsLoopback(pageHost)
  ) { apiBase = fromScriptOrigin; }
  ...
  function applySparkThemeFromId(themeId, done) {
    ...
    function scopeCss(css) {
      var scopeA = '#zipline-spark-root';
      var scopeB = '#zipline-spark-sidebar-host';
      function alreadySparkScoped(psel) {
        return /^#zipline-spark-root\b/.test(psel) || /^#zipline-spark-sidebar-host\b/.test(psel);
      }
      return String(css).replace(/(^|})\s*([^@}{][^{]*)\{/g, function (m, brace, sel) {
        ...
        for (var i = 0; i < parts.length; i++) {
          var psel = parts[i].trim();
          if (!psel) continue;
          if (alreadySparkScoped(psel)) { out.push(psel); }
          else { out.push(scopeA + ' ' + psel); out.push(scopeB + ' ' + psel); }
        }
        return (brace || '') + '\n' + out.join(',\n') + ' {\n';
      });
    }
    fetch(apiBase + '/api/v1/prompts/themes/' + encodeURIComponent(String(tid)), { credentials: 'omit' })
      .then(function (r) {
        if (!r.ok) {
          if (console && console.warn) console.warn('[Zipline Spark] Theme fetch failed (theme id=' + tid + ', HTTP ' + r.status + ').');
          return null;
        }
        return r.json();
      })
      .then(function (resp) {
        ...
        if (css && typeof css === 'string' && css.trim()) {
          var el = document.getElementById('zipline-spark-ab-theme');
          if (el) el.remove();
          var style = document.createElement('style');
          style.id = 'zipline-spark-ab-theme';
  ...
  function fetchSparkAssign(done) {
    ...
    if (p.pubid) body.pubid = p.pubid;
    if (fv) body.force_variant = fv;
    fetch(apiBase + '/api/v1/ab-tests/spark/assign', {
      method: 'POST',
      ...
      .then(function (data) {
        ...
        if (typeof cfg === 'string') {
          try { cfg = JSON.parse(cfg); if (typeof cfg === 'string') { try { cfg = JSON.parse(cfg); } catch (e2) { cfg = null; } } } catch (e) { cfg = null; }
        }
        if (cfg && typeof cfg === 'object' && !Array.isArray(cfg)) {
          if (cfg.psid) p.afs_style_id = String(cfg.psid).trim();
          var rawTheme = cfg.theme != null && cfg.theme !== '' ? cfg.theme : cfg.theme_id;
          if (rawTheme != null && rawTheme !== '') {
            var th = parseInt(String(rawTheme), 10);
            if (th > 0) window.__ziplineSparkAbThemeId = th;
          }
        }
        done();
      })
  ...
  function applyDDLayout(liveRoot, questions, openSidebarFn) {
    ...
    /* 3. Input bar */
    var firstQ = questions && questions[0] ? String(questions[0]) : '';
    ...
    function fire(q) {
      q = (q || '').trim();
      if (!q) return;
      /* Find a rendered card whose text matches; fall back to openSidebarFn */
    ...
    submitBtn.addEventListener('click', function () { fire(input.value); });
    input.addEventListener('keydown', function (e) { if (e.key === 'Enter') fire(input.value); });

    /* Sync input text when a card is clicked */
    if (carousel) {
      carousel.addEventListener('click', function (e) {
        var card = e.target.closest('.zipline-spark__card');
        if (!card) return;
        var ct = card.querySelector('.zipline-spark__card-text');
  ...
  var sparkScrollLockTop = 0;
  var sparkScrollLocked = false;

  var sparkPageLoadTs = Date.now();
  var trackingUrl =
    (typeof window !== 'undefined' && window.SPARK_TRACKING_URL) ||
    'https://e.nativemetrics-svc.com/event';
  var sparkVariation = 'Zipline Spark';
  var resolvedClientIP = '';
  var resolvedClientIPReady = null;
  var resolvedClientIPLoaded = false;
  ...
  function fetchPublicClientIPFallback() {
    return fetch('https://api.ipify.org?format=json', { method: 'GET', credentials: 'omit', headers: { 'Accept': 'application/json' } })
      ...
      .then(function (data) { if (data && typeof data.ip === 'string' && data.ip.trim()) resolvedClientIP = data.ip.trim(); })
      .catch(function () {});
  }
  ...
  function getSparkUserId() {
    var domain = '';
    try { domain = new URL(pageUrl()).hostname; } catch (e) { domain = window.location.hostname || 'default'; }
    var key = 'zipline_spark_uid_' + domain;
    try {
      var existing = localStorage.getItem(key);
      if (existing) return existing;
      var id = 'spark_u_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 11);
  ...
  function resolveChannelIdForSpark() {
    ...
    if (!raw) {
      try {
        ...
        raw = (sp.get('channel_id') || sp.get('channelid') || '').trim();
      } catch (e) { raw = ''; }
    }
    var s = String(raw);
  ...
  function getTrackingPayload(extra) {
    var urlParams = {};
    try { new URLSearchParams(window.location.search).forEach(function (v, k) { urlParams[k] = v; }); } catch (e) {}
    var domain = '';
    try { domain = new URL(pageUrl()).hostname; } catch (e2) { domain = window.location.hostname || ''; }
    var base = {
      timestamp: new Date().toISOString(),
      url: pageUrl(), domain: domain, referrer: document.referrer || '',
      url_parameters: urlParams, user_agent: navigator.userAgent, language: navigator.language,
      variation: sparkVariation, session_id: sessionId(), user_id: getSparkUserId(),
      channel_id: resolveChannelIdForSpark(), pubid: (p.pubid || '').trim(), api_base: apiBase
    };
    for (var si = 1; si <= 5; si += 1) {
      var subid = (p['subid' + si] || '').trim();
      if (subid) base['subid' + si] = subid;
    }
  ...
  function sendSparkEvent(eventType, data, useBeacon) {
    if (!resolvedClientIPLoaded && !useBeacon) {
      fetchResolvedClientIP().finally(function () { sendSparkEvent(eventType, data, useBeacon); });
      return;
    }
    var payload = getTrackingPayload(data || {});
    var qs = '';
    try { qs = new URLSearchParams(window.location.search).toString(); } catch (e) {}
  ...
  function ensureSparkAdMessageListener() {
    if (sparkAdMessageListenerAdded) return;
    sparkAdMessageListenerAdded = true;
    window.addEventListener('message', function (e) {
      var okOrigins = ['https://www.google.com','https://www.adsensecustomsearchads.com','https://syndicatedsearch.goog'];
      if (okOrigins.indexOf(e.origin) === -1) return;
      var s = e.data;
      if (!s || typeof s !== 'string' || s.indexOf('FSXDC,.aCS') !== 0) return;
  ...
  function reportSparkPageDuration() {
    if (sparkPageDurationReported) return;
    sparkPageDurationReported = true;
  ...
  function ensureSidebar() {
    ...
    host.innerHTML =
      ...
      '      <span class="zipline-spark__sidebar-star" aria-hidden="true">' + boltSvgMarkup('zipline-spark__bolt-icon') + '</span>' +
      '      <span>' + escapeHtml(p.title) + '</span></div>' +
      '    <button type="button" class="zipline-spark__sidebar-close" data-spark-close aria-label="Close">\xd7</button>' +
      '  </div>' +
  ...
  function isSparkScrollableNode(node) {
    while (node && node !== document.body) {
      if (node === chatScroll || node === paneAnswer || node === paneSources ||
        (node.classList && (node.classList.contains('zipline-spark__chat') || node.classList.contains('zipline-spark__pane') || node.classList.contains('zipline-spark__pane--answer') || node.classList.contains('zipline-spark__pane--sources')))) {
        return true;
      }
  ...
  function stopBackgroundScroll(ev) {
    if (!sidebarEl || sidebarEl.hidden) return;
    if (!isSparkScrollableNode(ev.target)) { ev.preventDefault(); ev.stopPropagation(); }
  }

  function lockPageScrollForSpark() {
    if (sparkScrollLocked) return;
    sparkScrollLocked = true;
    ...
    document.body.style.top = '-' + sparkScrollLockTop + 'px';
    document.body.style.left = '0'; document.body.style.right = '0'; document.body.style.width = '100%';
  }

  function unlockPageScrollForSpark() {
    if (!sparkScrollLocked) return;
    sparkScrollLocked = false;
  ...
  function injectAfsHeadOnce() {
    ...
    s1.setAttribute('data-zipline-spark-afs', '1'); document.head.appendChild(s1);
    var s2 = document.createElement('script'); s2.type = 'text/javascript'; s2.charset = 'utf-8';
    s2.setAttribute('data-zipline-spark-afs', 'bootstrap');
    s2.textContent = "(function(g,o){g[o]=g[o]||function(){(g[o]['q']=g[o]['q']||[]).push(arguments)},g[o]['t']=1*new Date})(window,'_googCsa');";
  ...
  function getSparkAfsNoFillDiagnostics(question, containerId) {
    ...
    var iframeURL = null; var iframeNum = ''; var iframeScStatus = '';
    if (iframe && iframe.src) {
      try { iframeURL = new URL(iframe.src); iframeNum = iframeURL.searchParams.get('num') || ''; iframeScStatus = iframeURL.searchParams.get('sc_status') || ''; } catch (e) {}
    }
    if ((!iframeNum || !iframeScStatus) && iframe && iframe.name) {
      try {
        var raw = iframe.name.indexOf('|') !== -1 ? iframe.name.split('|').pop() : iframe.name;
        var parsed = JSON.parse(raw);
        var master = parsed && (parsed['master-1'] || parsed.master);
        if (master) { if (!iframeNum && master.num != null) iframeNum = String(master.num); if (!iframeScStatus && master.sc_status != null) iframeScStatus = String(master.sc_status); }
      } catch (e2) {}
    }
    var rect = iframe ? iframe.getBoundingClientRect() : null;
    var containerRect = container ? container.getBoundingClientRect() : null;
    var iframeHeight = rect ? Math.round(rect.height) : null;
    var iframeVisibility = iframeStyle ? iframeStyle.visibility : '';
    var iframeDisplay = iframeStyle ? iframeStyle.display : '';
    var suspectedNoFill = !!iframe && (iframeHeight === 0 || iframe.offsetHeight === 0 || iframeVisibility === 'hidden' || iframeDisplay === 'none' || iframeNum === '0' || iframeScStatus === '6');
    return {
      container_id: containerId || '', query: question || '', iframe_exists: !!iframe,
      iframe_count: container ? container.querySelectorAll('iframe').length : 0,
      iframe_height: iframeHeight, iframe_offset_height: iframe ? iframe.offsetHeight : null,
      iframe_client_height: iframe ? iframe.clientHeight : null,
      iframe_width: rect ? Math.round(rect.width) : null,
      iframe_visibility: iframeVisibility, iframe_display: iframeDisplay,
      iframe_style_height: iframe ? iframe.style.height || '' : '',
      iframe_style_visibility: iframe ? iframe.style.visibility || '' : '',
      iframe_num: iframeNum, iframe_sc_status: iframeScStatus,
  ...
  function trackSparkAfsNoFillIfSuspected(question, containerId, extra) {
    var diagnostics = getSparkAfsNoFillDiagnostics(question, containerId);
    if (!diagnostics.suspected_no_fill) return false;
    sendSparkEvent('spark_bot_afs_no_fill', Object.assign({}, diagnostics, extra || {}));
    return true;
  }
  ...
  function appendTurn(question) {
    ...
    var adBlock =
      ...
      '    <div class="zipline-spark__afs-container" id="' + afsDomId + '" aria-hidden="true"></div>' +
      '  </div>';
    turn.innerHTML =
      ...
      '    <button type="button" class="zipline-spark__copy-btn zipline-spark__turn-copy" title="Copy answer">Copy</button>' +
      '  </div>' +
      adBlock +
  ...
  function monetizeSparkTurn(question, turnEl) {
    ...
    if (hasAds) {
      ...
      var containerId = afsEl && afsEl.id ? afsEl.id : '';
      var qtext = (question || '').trim() || (document.title || '').trim() || '';
      setTimeout(function () { sendSparkEvent('spark_bot_ad_viewed', { query: qtext, container_id: containerId }); }, 1000);
      setTimeout(function () { trackSparkAfsNoFillIfSuspected(qtext, containerId, { check_after_ms: 2500 }); }, 2500);
      setTimeout(function () { trackSparkAfsNoFillIfSuspected(qtext, containerId, { check_after_ms: 6000 }); }, 6000);
    }
  }
  ...
  function applyAnswerTruncation(ansEl) {
    ...
    if (inner.scrollHeight <= maxBeforeClamp) {
      while (inner.firstChild) { ansEl.insertBefore(inner.firstChild, inner); }
      inner.remove(); return;
    }
    ansEl.classList.add('zipline-spark__answer--collapsed');
    var btn = document.createElement('button');
    btn.type = 'button'; btn.className = 'zipline-spark__show-more-btn'; btn.textContent = 'Show more';
  ...
  function dispatchSSEBlock(block) {
    ...
    if (ev === 'chunk' && data.t) {
      ...
      if (firstChunk) {
        ...
        if (turnRoot && streamTarget) monetizeSparkTurn(streamTarget.question, turnRoot);
      }
      var acc = (ansEl.getAttribute('data-raw') || '') + data.t;
      ansEl.setAttribute('data-raw', acc);
      setAnswerHtmlStreaming(ansEl, basicMd(acc));
      if (firstChunk) scrollChatElementToTop(ansEl);
    }
    if (ev === 'error') {
      ...
      if (errTurn && streamTarget) monetizeSparkTurn(streamTarget.question, errTurn);
      scrollChatElementToTop(ansEl);
    }
    if (ev === 'done') {
      var st = streamTarget;
      var qStr = st && st.question;
      var start = st && st.answerStartMs;
      ...
      if (qStr && finalAnswer) {
        conversationHistory.push({ question: qStr, answer: String(finalAnswer) });
        if (conversationHistory.length > 12) conversationHistory = conversationHistory.slice(conversationHistory.length - 12);
      }
    ...
    if (ev === 'followups' && data.questions && data.questions.length) {
      ...
      data.questions.forEach(function (q) {
        if (!q) return;
        h += '<button type="button" class="zipline-spark__explore-chip">' + escapeHtml(q) + '<span class="zipline-spark__explore-arrow">\u21a9</span></button>';
      });
      explEl.innerHTML = h;
  ...
  function runStreamIntoTurn(question, turnEl) {
    ...
    fetch(apiBase + '/api/v1/spark/answer', {
      ...
      body: JSON.stringify((function () {
        ...
        var fv = forceVariantFromPage();
        if (fv) payload.force_variant = fv;
        return payload;
      })()),
      ...
      .then(function (res) {
        if (!res.ok) {
          return res.json().then(
            function (j) { throw new Error((j && j.detail) || (j && j.error) || 'HTTP ' + res.status); },
        ...
        function pump() {
          return reader.read().then(function (r) {
            if (r.done) {
              if (buf.trim()) { var sep = buf.indexOf('\n\n'); if (sep >= 0) dispatchSSEBlock(buf.slice(0, sep)); }
              if (streamTarget && streamTarget.ansEl) applyAnswerTruncation(streamTarget.ansEl);
              streamTarget = null; return;
            }
      ...
      .catch(function (e) {
        ...
        if (fetchWrap) { fetchWrap.hidden = false; monetizeSparkTurn(question, turnEl); }
  ...
  function fetchQuestionsWithRetry() {
    ...
    return fetchQuestionsOnce().then(parseQuestions)
      .catch(function () { return delay(500).then(fetchQuestionsOnce).then(parseQuestions); })
      .catch(function () { return delay(900).then(fetchQuestionsOnce).then(parseQuestions); });
  }

  function sparkHeadHtml() {
    return (
      '<div class="zipline-spark__head">' +
      '<span class="zipline-spark__head-star" aria-hidden="true">' + boltSvgMarkup('zipline-spark__bolt-icon') + '</span>' +
      '<span class="zipline-spark__head-title">' + escapeHtml(p.title) + '</span></div>'
    );
  }

  function renderQuestionCarousel(questions) {
    var liveRoot = getLiveRoot(themeClass, dw);
    if (root !== liveRoot && !root.isConnected) {
      if (console && console.warn) console.warn('[Zipline Spark] Original root was detached (hydration race). Using live element.');
    }
    liveRoot.innerHTML = '';
    ...
    head.innerHTML =
      '<span class="zipline-spark__head-star" aria-hidden="true">' + boltSvgMarkup('zipline-spark__bolt-icon') + '</span>' +
      '<span class="zipline-spark__head-title">' + escapeHtml(p.title) + '</span>';
    ...
    /* ── DD layout: runs after carousel is in the DOM ───── */
    applyDDLayout(liveRoot, questions, openSidebar);
  }

  function loadSparkQuestions() {
    fetchQuestionsWithRetry()
      .then(function (questions) { renderQuestionCarousel(questions); })
      .catch(function (err) {
        if (console && console.warn) console.warn('[Zipline Spark] questions failed after retries:', err && err.message ? err.message : err);
        var liveRoot = getLiveRoot(themeClass, dw);
        liveRoot.innerHTML = '';
  ...
  fetchSparkAssign(function () {
    withServerAfsDefaults(function () {
      ...
      if (th) {
        applySparkThemeFromId(th, function () { loadSparkQuestions(); });
      } else {
        loadSparkQuestions();
      }
    });
